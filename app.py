...
# ----------- Chain of Custody Routes (Synchronized with Templates) -----------

def _read_total_products_selected(saved_path: str) -> pd.DataFrame:
    """Read Total Products file and keep ONLY the columns you listed.

    Assumptions (per your note):
      - Row 1 is the header row (Excel row 1) -> pandas row index 0
      - Data starts on row 2 -> pandas row index 1
      - Columns are fixed by position:
          B=Product Name, E=Anticipated Chemical, G=Received On, H=Received By,
          I=Laboratory ID, O=ASIN (Identifier), T=Client, U=Phone, V=Email,
          W=Project Lead, X=Address
    """
    raw = None
    last_err = None
    for loader in (
    ...
    if raw is None:
        raise ValueError(f"Could not read file: {last_err}")

    raw = raw.fillna("")

    # 0-based indexes for Excel columns (A=0)
    col_idxs = [1, 4, 6, 7, 8, 14, 19, 20, 21, 22, 23]
    needed_headers = [
        "Product Name",
    ...
    max_idx = max(col_idxs)
    if raw.shape[1] <= max_idx:
        raise ValueError(
            f"File has only {raw.shape[1]} columns, but we need at least {max_idx+1} columns (through Excel column X).\n"
            "Make sure you're uploading the correct Total Products export."
        )

    header_row = raw.iloc[0, col_idxs].tolist()
    # If the file already has these exact headers, keep them; otherwise enforce ours.
    header_norm = [str(x).strip().lower() for x in header_row]
    needed_norm = [h.strip().lower() for h in needed_headers]
    use_headers = needed_headers if header_norm != needed_norm else needed_headers

    df = raw.iloc[1:, col_idxs].copy()
    df.columns = use_headers
...
def _ingest_total_products_for_coc(df: pd.DataFrame, u: Dict[str, Any], filename: str) -> str:
    """Upsert coc_records from a Total Products file using ONLY the selected columns."""
    df = df.fillna("").copy()

    idx_product_name = _find_exact(list(df.columns), "Product Name")
    idx_anticipated = _find_exact(list(df.columns), "Anticipated Chemical")
    idx_received_on = _find_exact(list(df.columns), "Received On")
    idx_received_by_file = _find_exact(list(df.columns), "Received By")
    idx_lab = _find_exact(list(df.columns), "Laboratory ID")
    idx_asin = _find_exact(list(df.columns), "ASIN (Identifier)")
    idx_client = _find_exact(list(df.columns), "Client")
    idx_phone = _find_exact(list(df.columns), "Phone")
    idx_email = _find_exact(list(df.columns), "Email")
    idx_project_lead = _find_exact(list(df.columns), "Project Lead")
    idx_address = _find_exact(list(df.columns), "Address")

    if idx_lab is None:
        return "COC import failed: Missing required column 'Laboratory ID' (Column I)."

    def get(row, idx: Optional[int]) -> str:
        if idx is None:
            return ""
    ...
    # IMPORTANT: Prevent UNIQUE constraint failures when the same Lab ID repeats in the same upload
    seen: Dict[str, ChainOfCustody] = {}

    try:
        for _, row in df.iterrows():
            raw_lab = get(row, idx_lab)
            if not raw_lab:
                skipped += 1
                continue
            ...
            client_name = get(row, idx_client) or (u.get("client_name") if u.get("role") == "client" else CLIENT_NAME)
            sample_name = get(row, idx_product_name)
            asin = get(row, idx_asin)
            anticipated_chemical = get(row, idx_anticipated)
            phone = get(row, idx_phone)
            email = get(row, idx_email)
            project_lead = get(row, idx_project_lead)
            address = get(row, idx_address)

            received_on_raw = get(row, idx_received_on)
            received_by_raw = get(row, idx_received_by_file)
            ...
            # Do not auto-receive unless the file explicitly has it AND record isn't already checked in
    ...
    return f"COC Intake complete: {created} created, {updated} updated, {skipped} skipped (missing/invalid Lab ID)."


@app.route("/coc")
def coc_list():
    """Main Chain of Custody table. Matches url_for('coc_list')"""
    u = current_user()
    if not u.get("username"):
        return redirect(url_for("home"))
...
def coc_import_total_products():
    ...
    try:
        df = _read_total_products_selected(saved_path)
        msg = _ingest_total_products_for_coc(df, u, filename)
        flash(msg, "success" if not msg.lower().startswith("coc import failed") else "error")
    except Exception as e:
        flash(f"COC import failed: {e}", "error")
    finally:
        if os.path.exists(saved_path) and not KEEP_UPLOADED_CSVS:
            os.remove(saved_path)
...
def coc_receive_selected():
    ...
    try:
        for rid in selected_ids:
            ...
            if u.get("role") != "admin" and rec.received_at is not None:
                already_received += 1
                continue
...
def coc_print_single(record_id):
    """Matches url_for('coc_print_single')"""
    u = current_user()
    if not u.get("username"):
        return redirect(url_for("home"))
...
def coc_pdf_single(record_id):
    ...
    try:
        ...
        pdf_io = _build_coc_pdf([rec], title=f"Chain of Custody - {rec.lab_id or rec.id}")
        out_name = f"COC_{(rec.lab_id or record_id)}.pdf"
        return send_file(pdf_io, mimetype="application/pdf", as_attachment=True, download_name=out_name)
    finally:
        db.close()
...
def coc_edit(record_id):
    ...
    try:
        ...
        if request.method == "POST":
            ...
            record.sample_condition = request.form.get("sample_condition")
            record.anticipated_chemical = request.form.get("anticipated_chemical")
            log_action(u.get("username"), u.get("role"), "COC_UPDATE", f"Sample {record.lab_id} updated")
            db.commit()
